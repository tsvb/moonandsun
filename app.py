...
# Signs grouped by element for chart balance analysis
ELEMENT_SIGNS = {
...
def generate_aspect_interpretation(aspect_data):
    """Provide text interpretations for aspects."""
    asp = aspect_data.get("aspect")
    keywords = ASPECTS_INFO.get(asp, {}).get("keywords", "")
    p1 = aspect_data.get("planet1")
    p2 = aspect_data.get("planet2")
    text = f"The {asp.lower()} between {p1} and {p2}"
    if keywords:
        text += f" suggests {keywords}"
    return text + "."


def analyze_chart_emphasis(positions):
    """Detect elemental and modal balance and chart shape."""
    element_balance = {e: 0 for e in ELEMENT_SIGNS}
    modality_balance = {m: 0 for m in MODALITY_SIGNS}
    for lon in positions.values():
        idx = int(lon // 30) % 12
        sign = ZODIAC_SIGNS[idx]
        element = next(e for e, signs in ELEMENT_SIGNS.items() if sign in signs)
        modality = next(m for m, signs in MODALITY_SIGNS.items() if sign in signs)
        element_balance[element] += 1
        modality_balance[modality] += 1

    lons = list(positions.values())
    if len(lons) < 2:
        shape = "undefined"
    else:
        lons.sort()
        gaps = [(lons[(i + 1) % len(lons)] - lons[i]) % 360 for i in range(len(lons))]
        span = 360 - max(gaps)
        if span <= 120:
            shape = "bundle"
        elif span <= 180:
...
def generate_personality_keywords(positions, aspects=None):
    """Extract key themes from chart."""
    info = analyze_chart_emphasis(positions)
    element = max(info["element_balance"], key=info["element_balance"].get)
    modality = max(info["modality_balance"], key=info["modality_balance"].get)
    keywords = [f"{element} emphasis", f"{modality} nature"]
    if aspects:
        major = [a for a in aspects if a.get("type") == "major"]
        major.sort(key=lambda a: a["strength"], reverse=True)
        for asp in major[:3]:
            kw = ASPECTS_INFO.get(asp["aspect"], {}).get("keywords", "")
            if kw:
                keywords.append(f"{asp['planet1']}-{asp['planet2']} {kw}")
    return keywords
...
def draw_chart_wheel(
    ...
    try:
        ...
        for i, glyph in enumerate(ZODIAC_GLYPHS):
            ...
            ax.text(
                x, y, glyph, ha="center", va="center", fontsize=12, color=THEME["fg"]
            )
